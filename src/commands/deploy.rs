...
pub struct DeployCommand {
    ...
    #[arg(long, help = "The number of allowed the SFT attributes repeats. 0 means do not limit.", default_value = "0")]
    repeat: u64,
...
impl DeployCommand {
    pub fn run(self, wallet: Wallet) -> SubcommandResult {
        ...
        let output = Inscriber::new(wallet, self.inscribe_options)?
            .with_deploy(tick, self.amount, self.generator, self.repeat, self.deploy_args)?
            .inscribe()?;
        Ok(Box::new(output))
    }
}
